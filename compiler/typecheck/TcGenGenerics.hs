...
following constraints are satisfied.
  ...
  (e) For any type-level application (Tfun Targ) in the right-hand side of D
      ...
      (e1) `a' must not occur in Tfun.

      (e2) If `a' occurs in Targ, then Tfun :: * -> *.
...
canDoGenerics1 rep_tc =
  ...
  where
    ...
    check_con con = case check_vanilla con of
      j@(NotValid {}) -> [j]
      IsValid -> foldDataConArgs (ft_check con) con
    ...
    -- See also Note [FFoldType and foldType] in TcGenDeriv
    ft_check :: DataCon -> FFoldType Validity
    ft_check con = SimpleFT
      { ft_triv = IsValid, ft_var = IsValid

      -- (ty arg), where the parameter of interest does not occur in ty
      , ft_ty_app = \_ arg -> arg

      --  check (e1) from Note [Requirements for deriving Generic and Rep]
      , ft_bad_app = NotValid $ bad con wrong_arg
