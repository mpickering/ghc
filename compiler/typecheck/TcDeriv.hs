...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
TODO: This needs to be carefully reworded to avoid using the phrase
"functor-like" erroneously.
...
-- variables and instance types, in case their kinds were refined due to
-- the presence of type class constraints.
-- See Note [Inferring the instance context]
  ...
  where
    ...
    con_arg_constraints get_arg_constraints mkTheta
      = let (predss, mbSubsts) = unzip
            ...
            -- (e.g., if it needs a Functor or Traversable constraint), then we
            -- explicitly unify the subtype's kind with (* -> *).
            -- See Note [Inferring the instance context]
    ...
    -- is_star_to_star: see Note [Inferring the instance context]
    is_star_to_star = typeKind inst_ty `tcEqKind` typeToTypeKind
                   || is_generic1 -- Technically, Generic1 requires a type of
                                  -- kind (k -> *), not (* -> *), but we still
                                  -- label it "star-to-star" to make sure
    ...
    get_gen1_constraints functor_cls orig t_or_k ty
       = mk_star_to_star_constraints orig t_or_k functor_cls $
    ...
    get_std_constrained_tys orig t_or_k ty
        | is_star_to_star = mk_star_to_star_constraints orig t_or_k main_cls $
                            deepSubtypesContaining last_tv is_functor_like ty
        | otherwise       = [( [mk_cls_pred orig t_or_k main_cls ty]
                             , Nothing )]

    -- The Functor, Foldable, and Traversable classes require slightly
    -- different sets of constraints than other type classes of kind * -> *.
    -- See Note [FFoldType and foldType] in TcGenDeriv
    is_functor_like :: Bool
    is_functor_like = getUnique main_cls `elem` functorLikeClassKeys

    mk_star_to_star_constraints :: CtOrigin -> TypeOrKind
                                -> Class -> [Type]
                                -> [(ThetaOrigin, Maybe TCvSubst)]
    ...
    -- See Note [Inferring the instance context]
    mk_star_to_star_constraints orig t_or_k cls
    ...
    all_rep_tc_args | is_star_to_star = rep_tc_args ++ [mkTyVarTy last_tv]
