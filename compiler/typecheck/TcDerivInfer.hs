...
inferConstraints tvs main_cls cls_tys inst_ty rep_tc rep_tc_args
  ...
  = thing [ThetaOrigin [] []] tvs inst_tys
  ...
  where
    ...
    con_arg_constraints get_arg_constraints thing
      = let (predss, mbSubsts) = unzip
            ...
            preds = ThetaOrigin [] $ concat predss
    ...
    extra_constraints = [ThetaOrigin [] constrs]
      where
        constrs
...
inferConstraintsDAC cls tvs inst_tys thing =
  let theta_origins
        = [ mkThetaOrigin DerivOrigin TypeLevel orig_theta' gm_theta'
          | (sel_id, Just (_, GenericDM gm_ty)) <- classOpItems cls
            -- In the code below, things prefixed with "orig_" refer to the
            -- non-default type signature, and things prefixed with "gm" refer
            -- to the default type signature.
          , let orig_ty = thdOf3 $ tcSplitMethodTy (varType sel_id)
                (_, orig_theta, orig_tau) = tcSplitSigmaTy orig_ty
                (_, gm_theta,   gm_tau)   = tcSplitSigmaTy gm_ty
                ...
                in_scope  = mkInScopeSet $ tyCoVarsOfTypes $ gm_ty:inst_tys
                tv_env    = zipVarEnv (classTyVars cls) inst_tys
                subst     = mkTvSubst in_scope tv_env
                gm_theta' = substTheta subst gm_theta
                gm_tau'   = substTy subst gm_tau
                ...
                --     default quux :: forall b. Eq (f b) => f b -> f b -> Bool
                --
                -- We need a way to treat `a` and `b` as the same. So to wrap
                -- up inferConstraintsDAC, we unify the default and non-default
                -- type signatures and apply the resulting substitution.
                -- TODO RGS: This doesn't work. See the TODO below.
                mb_gm_subst = tcUnifyTy orig_tau gm_tau'
                -- TODO RGS: This is completely broken. Ideally, if the default
                -- and non-default type signatures do not match, we should
                -- throw an error similar to the one that is thrown if you
                -- were to write out the derived instance manually.
                gm_subst    = fromMaybe (pprPanic "inferConstraintsDAC" $
                                            vcat [ppr orig_tau, ppr gm_tau'])
                                        mb_gm_subst
                orig_theta' = substTheta gm_subst orig_theta
                ]
...
simplifyDeriv pred tvs thetas
  = do { (skol_subst, tvs_skols) <- tcInstSkolTyVars tvs -- Skolemize
       ...
       ; let skol_set  = mkVarSet tvs_skols
             ...
             mk_given_ct :: CtLoc -> PredType -> TcM CtEvidence
             mk_given_ct loc given = do
               let given_pred = substTy skol_subst given
               new_ev <- newEvVar given_pred
               pure CtGiven { ctev_pred = given_pred
                            , ctev_evar = new_ev
                            , ctev_loc  = loc }

             mk_wanted_ct :: PredOrigin -> TcM CtEvidence
             mk_wanted_ct (PredOrigin wanted o t_or_k)
               = newWanted o (Just t_or_k) (substTy skol_subst wanted)

             -- Decompose a ThetaOrigin into its component given and
             -- wanted constraints.
             mk_cts :: ThetaOrigin -> TcM [CtEvidence]
             mk_cts (ThetaOrigin { to_givens = givens
                                 , to_wanted_origins = wanteds }) = do
               lcl_env <- getLclEnv
               let given_loc = mkGivenLoc (tcl_tclvl lcl_env) skol_info lcl_env
               wanted_cts <- mapM mk_wanted_ct wanteds
               given_cts  <- mapM (mk_given_ct given_loc) givens
               pure $ wanted_cts ++ given_cts

       -- Generate the given and wanted constraints with the skolemized
       -- variables
       ; (cts, tclvl) <- pushTcLevelM (mapM mk_cts thetas)

       ; traceTc "simplifyDeriv inputs" $
         vcat [ pprTyVars tvs $$ ppr thetas $$ ppr cts, doc ]
       -- Now we need to simplify the constraints. First, we simplify each set
       -- of constraints per ThetaOrigin separately...
       ; residual_cts <- mapM simplifyWantedsTcM cts
       ; traceTc "simplifyDeriv (after one simplifer pass)" $
         ppr residual_cts
       -- ...and after one simplifcation pass, we combine all of the wanted
       -- constraints and pass them through the simplifer one more time.
       -- See Note [Simplifying constraints for DeriveAnyClass]
       ; (residual_cts', _) <- runTcS $ solveWantedsAndDrop
                                      $ unionsWC residual_cts
       ; traceTc "simplifyDeriv (after two simplifer passes)" $
         ppr residual_cts'
       ; residual_cts_zonked <- zonkWC residual_cts'
       ...
       ; let residual_simple = wc_simple residual_cts_zonked
             (bad, good) = partitionBagWith get_good residual_simple
             unsolved    = residual_cts_zonked { wc_simple = bad }
             ...
             get_good ct | validDerivPred skol_set p
                         , isWantedCt ct
                         = Right p
                          -- NB re 'isWantedCt': residual_cts_zonked may
                          -- contain unsolved CtDerived and we stick them into
                          -- the bad set so that reportUnsolved may decide what
                          -- to do with them
                         | otherwise
                         = Left ct
                           where p = ctPred ct
...
Note [Simplifying constraints for DeriveAnyClass]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
For stock and newtype deriving, you collect only wanted constraints, so it
suffices to pass them all to simplifier once. But with DeriveAnyClass, you
end up needing to invoke the simplifier twice! Why? Suppose you derive a
Foo (Maybe a) instance using DeriveAnyClass, where
...
You'll end up with two sets of constraints to simplify:

  bar: (Givens: [],             Wanteds: [Show (Maybe a)])
  baz: (Givens: [Eq (Maybe a)], Wanteds: [Ord (Maybe a), Show (Maybe a)])

First, note that these sets of constraints need to be simplified separately,
as the (Eq (Maybe a)) given constraint for baz should not be used to simplify
the constraints for bar. After simplification, you get:

  bar: (Givens: [],             Wanteds: [Show a])
  baz: (Givens: [Eq (Maybe a)], Wanteds: [Ord a, Show a])

It seems that we can now just lump up all of the wanted constraints together to
form the instance context. But wait! If we do that, we'll get this instance:

  instance (Show a, Ord a, Show a) => Foo (Maybe a)

We don't want those duplicate (Show a) constraints, since class instances are
frequently looked up by their _exact_ instance context (e.g., hs-boot files do
this). So we take all of the wanted constraints and pass them through the
simplifier once more to remove duplicate constraints, leaving us with the
correct instance context:

  instance (Ord a, Show a) => Foo (Maybe a)
