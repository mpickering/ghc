...
inferConstraints :: Class -> [TyVar] -> [Type]
                 -> DerivSpecMechanism
...
inferConstraints main_cls tvs inst_tys mechanism
  = do { (arg_constraints, tvs', inst_tys') <- infer_constraints
       ; traceTc "inferConstraints" $ vcat
              [ ppr main_cls <+> ppr inst_tys'
              , ppr arg_constraints
              ]
       ; return ( sc_constraints ++ arg_constraints
                , tvs', inst_tys' ) }
  where
    infer_constraints
      | DerivSpecStock { dsm_inst_args = inst_args } <- mechanism
      = infer_constraints_data_con_args inst_args
      | DerivSpecNewtype { dsm_inst_args = inst_args } <- mechanism
      = infer_constraints_data_con_args inst_args
      | DerivSpecAnyClass <- mechanism
      = inferConstraintsDAC main_cls tvs inst_tys

    infer_constraints_data_con_args
      = inferConstraintsDataConArgs main_cls tvs inst_tys
...
inferConstraintsDataConArgs :: Class -> [TyVar] -> [Type]
