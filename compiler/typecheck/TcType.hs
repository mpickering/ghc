...
-- applied types. Note that this may fail in the case of an ill-kinded 'FunTy'
-- (e.g. @FunTy (a :: k) Int@, since the kind of @a@ isn't of the form
-- @TYPE rep@). Consequently, you may need to zonk your type before
-- using this function.
--
...
tc_eq_type view_fun orig_ty1 orig_ty2 = go True orig_env orig_ty1 orig_ty2
  where
    ...
    -- sometimes hard to know directly because @ty@ might have some casts obscuring
    -- the FunTy. And 'splitAppTy' is difficult because we can't always extract a
    -- RuntimeRep (see Note [xyz]) if the kind of the arg or res is
    -- unzonked/unflattened. Thus this function, which handles this corner case.
