...
tc_single _top_lvl sig_fn _prag_fn (L _ (PatSynBind psb@PSB{ psb_id = L _ name })) thing_inside
  ...
  where
    tc_pat_syn_decl = case sig_fn name of
        ...
        Just (TcPatSynSig tpsi) -> do
          let s_bndr = patsig_bndr tpsi
          case patsig_bndr tpsi of
            CompleteSig{} -> return ()
            PartialSig _ _ nwcs _ ->
...
mkExport prag_fn qtvs inferred_theta (poly_name, mb_sig, mono_id)
  = do  { mono_ty <- zonkTcType (idType mono_id)
        ; traceTc "mkExport" (ppr poly_name)
        ; (poly_id, inferred) <- case mb_sig of
              ...
              Just sig | Just poly_id <- completeIdSigPolyId_maybe sig
                       ...
                       | otherwise
                       -> do { let TISI { sig_bndr  = s_bndr
                                        , sig_theta = annotated_theta } = sig
                             ; final_theta <- completeTheta inferred_theta annotated_theta s_bndr
...
completeTheta :: TcThetaType -> TcThetaType -> TcIdSigBndr -> TcM TcThetaType
completeTheta inferred_theta annotated_theta s_bndr
...
tcTySig (L loc (PatSynSig (L _ name) (_, qtvs) prov req ty wcs))
  = setSrcSpan loc $
    tcWildcardBinders wcs $ \wc_prs ->
    do { traceTc "tcTySig {" $ ppr name $$ ppr qtvs $$ ppr prov $$ ppr req $$ ppr ty
       ...
       ; let sig_ty =
                      mkSigmaTy ex_tvs prov' $
                      mkSigmaTy univ_tvs req' $
                      ty'
             mbndr = mkBndr Nothing wc_prs sig_ty
       ; let tpsi = TPSI{ patsig_name = name,
                          ...
                          patsig_prov = prov',
                          patsig_req = req' }
       ; traceTc "mbdnr" (ppr mbndr)
       ; _ <- completeTheta [] [] mbndr
       ; return [TcPatSynSig tpsi] }}
  where
    mkBndr extra_cts nwcs sigma_ty
      | isNothing extra_cts && null nwcs
                  = CompleteSig (mkLocalId name sigma_ty)
      | otherwise
                  = PartialSig { sig_name = name, sig_nwcs = nwcs
                               , sig_cts = extra_cts, sig_hs_ty = ty }
