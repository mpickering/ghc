...
import Util
import Annotations
import BasicTypes( TopLevelFlag, isTopLevel, TypeOrKind(..))
import Maybes
...
checkThLocalIdX :: Id -> (Name -> Id -> TcM (Either PendingTcTypedSplice PendingZonkSplice))
                -> Name -> Id -> TcM Id
  ...
  = do  { mb_local_use <- getStageAndBindLevel n
        ; case mb_local_use of
             Just (top_lvl, bind_lvl, use_stage)
                ...
                -> checkCrossStageLifting mk_expr sp_id top_lvl n id use_stage
             _  -> return id   -- Not a locally-bound thing, or
                               -- no cross-stage link
    }

checkCrossStageLifting :: (Name -> Id -> TcM (Either PendingTcTypedSplice PendingZonkSplice))
...
checkCrossStageLifting mk_expr sp_id top_lvl id_name id
                        (Brack _ (TcPending ps_var zs_var lie_var wrap))
  | isTopLevel top_lvl
  = when (isExternalName id_name) (keepAlive id_name) >> return id
  ...
  =     -- Nested identifiers, such as 'x' in
    ...
    do  { -- Put the constraint (either Lift or LiftT) into the right LIE
        ; e <- setConstraintVar lie_var (mk_expr id_name id)
        ; case e of
            Left p -> updMutVar ps_var (p :)
            Right z -> updMutVar zs_var (z :)
        ; return sp_id}
checkCrossStageLifting _ _ _ _ id _ = return id
