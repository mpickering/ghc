...
FunCo mirrors FunTy in the Coercion type. Note how the RuntimeReps are implied
in the FunTy representation. For this reason we must be careful when
reconstructing the TyConApp representation (see, for instance,
splitTyConApp_maybe). In particular, splitTyConApp_maybe can only be used with
FunTys where both arguments are of the form @TYPE (r :: RuntimeRep)@.
