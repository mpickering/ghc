...
constraintIsLifted =
    CoAxiomRule { coaxrName = mkFastString "constraintIsLifted"
                , coaxrAsmpRoles = []
                , coaxrRole = Nominal
                , coaxrProves = const $ Just $ Pair constraintKind liftedTypeKind
                }
...
mkTyConAppCo r tc cos
  ...
  , [rep1, rep2, co1, co2] <- cos
  = -- (a :: TYPE ra) -> (b :: TYPE rb)  ~  (c :: TYPE rc) -> (d :: TYPE rd)
    -- rep1 :: ra  ~  rc        rep2 :: rb  ~  rd
    -- co1  :: a   ~  c         co2  :: b   ~  d
    --
    -- Sanity check: Ensure that RuntimeRep coercion arguments match actual
    -- kinds of 'a' and 'b'
    let co1_rep = mkNthCo 0 $ mkKindCo co1 -- extract RuntimeRep coercion
        co2_rep = mkNthCo 0 $ mkKindCo co2
        Pair co1a  co1b  = coercionKind co1_rep
        Pair co2a  co2b  = coercionKind co2_rep
        Pair rep1a rep1b = coercionKind rep1
        Pair rep2a rep2b = coercionKind rep2
        okay = True ||              -- TODO: Assert that rep cos make sense
             ( co1a `eqType` rep1a
            && co1b `eqType` rep1b
            && co2a `eqType` rep2a
            && co2b `eqType` rep2b)
        ppr_co co a b = ppr co <+> dcolon <+> ppr a <+> char '~' <+> ppr b
        err_doc = text "rep1" <+> ppr_co rep1    rep1a rep1b
               $$ text "rep2" <+> ppr_co rep2    rep2a rep2b
               $$ text "co1"  <+> ppr_co co1_rep co1a  co1b
               $$ text "co2"  <+> ppr_co co2_rep co2a  co2b
    in ASSERT2( okay, err_doc )
       mkFunCo r co1 co2
...
-- | Make a 'Coercion' quantified over either a type variable or
  ...
  where
    ...
    go (NthCo d co)
      ...
      = let (tc1,  args1) = splitTyConApp ty1
        ...
        in
        ASSERT2( tc1 == _tc2, ppr co0 $$ ppr d $$ ppr tc1 $$ ppr _tc2 )
