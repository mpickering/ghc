...
loadCoreExpr zs menv s =  pprTrace "LOADING" (ppr zs $$ ppr menv $$ ppr s) $ do
  env <- getGblEnv
  hs_env <- env_top <$> getEnv
  nc <- liftIO $ readIORef (hsc_NC hs_env)
  let
...
dsHsVar var
  ...
  | otherwise
 = do  mb_val <- dsLookupMetaEnv (getName var)
       case mb_val of
 --          Just (DsSpliceC e) -> return e
           _ -> return (varToCoreExpr var)   -- See Note [Desugaring vars]
