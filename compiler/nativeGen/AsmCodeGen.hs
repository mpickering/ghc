...
data NativeGenAcc statics instr
  = NGS { ngs_imports     :: ![[CLabel]]
        ...
        , ngs_unwinds     :: !(BlockEnv [UnwindPoint])
...
nativeCodeGen' dflags this_mod modLoc ncgImpl h us cmms
 = do
        ...
        bufh <- newBufHandle h
        let ngs0 = NGS [] [] [] [] [] [] emptyUFM emptyBlockMap
...
cmmNativeGen
    ...
    -> NcgImpl statics instr jumpDest
        ...
        -> IO   ( UniqSupply
                ...
                , BlockEnv [UnwindPoint]                    -- unwinding information for blocks
                )
...
computeUnwinding :: Instruction instr
                 ...
                 -> BlockEnv [UnwindPoint]
                    -- ^ unwinding tables for all points of all blocks of the
                    -- procedure
computeUnwinding dflags _ _
  | debugLevel dflags == 0         = emptyBlockMap
computeUnwinding _ _ (CmmData _ _) = emptyBlockMap
computeUnwinding _ ncgImpl (CmmProc _ _ _ (ListGraph blks)) =
    pushDownUnwinds
    $ mapFromList
        [ (blk_lbl, (generateUnwindTable ncgImpl instrs, succs))
        | BasicBlock blk_lbl instrs <- blks
        , let succs = concatMap jumpDestsOfInstr instrs ]
  where
    -- Here is where we push unwinding information from blocks down to
    -- their successors
    pushDownUnwinds :: BlockEnv ([UnwindPoint], [BlockId])
                    -> BlockEnv [UnwindPoint]
    pushDownUnwinds blks =
        foldl' (flip go) emptyBlockMap (mapToList blks)
      where
        go :: (Label, ([UnwindPoint], [BlockId]))
           -> BlockEnv [UnwindPoint]
           -> BlockEnv [UnwindPoint]
        go (lbl, (pts, _)) accum =
            mapInsert lbl pts accum
